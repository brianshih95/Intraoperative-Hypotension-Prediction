...
colors = ['red', 'orange', 'blue', 'cyan']  # draw roc curve
# Prespecifications
lr = 0.0001
...
for invasive in [True, False]:
    for multi in [False, True]:
        ...
        DNN = Net(task=task, invasive=invasive, multi=multi).to(device)
        ...
        for epoch in range(n_epochs):
            ...
            for dnn_inputs, dnn_target in loader['train']:
                ...
                print(dnn_output[:, 0].shape, dnn_target.shape)
                loss = criterion(dnn_output[:, 0], dnn_target)
                current_loss['train'] += loss.item() * dnn_inputs.size(0)
                loss.backward()
                optimizer.step()
            current_loss['train'] /= len(loader['train'].dataset)
        ...
        if task == "classification":
            ...
            roc_auc = auc(fpr, tpr)
            plt.plot(fpr, tpr, color=colors[color_idx], label='AUC: {:.3f}'.format(roc_auc))
        else:
            ...
            mae = np.mean(errors)
            ax.boxplot(errors, positions=[color_idx], patch_artist=True, showfliers=False,
                       boxprops=dict(facecolor='white', edgecolor='black'),
                       widths=0.5,
                       medianprops=dict(color=colors[color_idx]))
        color_idx += 1
